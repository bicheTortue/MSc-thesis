...
\begin{itemize}
  ...
  \item When the system is in parallel, the circuit's onChip area increases with a factor of $O(n_h)$.
\end{itemize}
...
\begin{equation}
  A_{xbar}(n_i,n_o,n_s)=2\cdot n_i\cdot n_o \cdot n_s\cdot A_{memristor}+2\cdot A_{CMOS}\cdot n_o\cdot n_s +n_o\cdot(2\cdot[A_{opAmp}+A_{R_r}]+A_{R_f})
\end{equation}
